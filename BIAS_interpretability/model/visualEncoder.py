...
class SELayer(nn.Module):
    ...
    def forward(self, x):  
        ...
        y = self.fc(y).view(c, 1, 1, 1)
        x = x * y
        x = x.transpose(1, 0) # Reshape to original
        return x, y
...
class ResNet(nn.Module):
    ...
    def forward(self, inputBatch):
        ...
        se_feat, se_vetor = self.se(batch)
        outputBatch = self.avgpool(se_feat)
        return outputBatch, se_vetor, se_feat
